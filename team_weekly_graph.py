...
from agents.team_weekly_report_generator import TeamWeeklyReportGenerator
from agents.wbs_data_retriever_agent import WBSDataRetrieverAgent
from core import config
from core.state_definition import TeamWeeklyLangGraphState
...
def load_wbs_node(state: TeamWeeklyLangGraphState) -> TeamWeeklyLangGraphState:
...
def generate_team_weekly_report_node(state: TeamWeeklyLangGraphState) -> TeamWeeklyLangGraphState:
    ...
    try:
        ...
        weekly_reports_data = state.get("weekly_reports_data")
        if not weekly_reports_data:
            raise ValueError("주간 보고서 데이터가 없습니다.")

        updated_state = generator.generate_team_weekly_report(state)
...
if __name__ == "__main__":
    ...
    try:
        test_app = create_team_weekly_graph()
