...
"summary": 제공된 일일 보고서 전체를 종합하여 한 주간의 핵심 성과와 진행 상황을 2~3문장으로 요약합니다.

"contents":

일주일 동안 수행된 여러 업무 중, 연관성이 높거나 중요한 활동들을 묶어 2~3개의 핵심 contents 항목으로 구성합니다.

각 contents 항목은 다음을 포함해야 합니다:

"text": 한 주간의 주요 업무 흐름을 자연스럽고 구체적으로 서술합니다.

"evidence": 위 "text" 내용에 대한 핵심적인 증거를 일일 보고서에서 1~2개 선별하여 요약합니다. title과 content는 일일 보고서의 evidence에서 가져와 재구성합니다.
...
"content"는 리스트 형식으로, 긍정적인 점, 아쉬운 점, 구체적인 개선 방안을 균형 있게 서술하세요.

중요 사항
모든 내용은 제공된 주간 일일 보고서 데이터를 기반으로 작성하세요.
...
weekly_reflection 섹션을 통해 한 주간의 업무에 대한 개인적 성찰과 다음 계획을 포함하세요.
...
{{
  ...
  "weekly_report": {{
    ...
    "contents": [
      {{
        "text": "(예시: **Git 데이터 기반 개인 업무 내용 파악 에이전트** 개발 작업을 주도적으로 진행했습니다. 주 초반에는 wbs_analyze_agent를 구현 및 테스트했으며, 주 중반에는 git analyzer와 wbs analyzer를 통합하는 PR을 통해 기능을 구체화했습니다.)",
        "evidence": [
           {{
              "title": "(예시: feat: [#1] wbs_analyze_agnet 구현 / Yax 18 git analyzer 개발)",
      ...
      {{
        "text": "(예시: 프로젝트 데이터의 **전처리 및 데이터베이스 적재** 작업을 수행했습니다. 특히 git, email 데이터의 전처리 및 VectorDB 적재 코드를 추가하고 리팩토링했습니다.)",
        "evidence": [
           {{
              "title": "(예시: Yax 33 git email 데이터 preprocessing 작업)",
...
5. Weekly reflection 섹션을 통해 당일 업무에 대한 개인적 성찰과 다음 계획을 포함하세요
