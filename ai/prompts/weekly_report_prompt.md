...
**모든 분석은 project_id 기준으로 분리하여 분석하세요.**
...
## Weekly_Reflection 생성
- **명사형 표현: 모든 문장을 명사형으로 마무리하여 간결하고 명확하게 작성**
Daily reports의 daily_reflection들을 분석하여 **주간 차원의 인사이트**를 도출:
- **가치 있는 인사이트 작성**: 인사이트는 객관적 데이터에 기반하여 작성하며, **업무 나열이 아닌** 업무**작업 패턴**, **협업 패턴**, 문제 해결 과정 등을 담은 가치 있는 인사이트를 작성. 
- **패턴 인식**: daily_reflection에 명시된 **장애물**, **업무 패턴**을 깊이 있게 작성. (**업무 패턴**이나 **개선점 포함**하여 작성) 업무 기여 나열보다는 패턴 기반 인사이트를 작성.
- **전략적 제안**: 이번 주 경험을 바탕으로 한 실용적인 개선 전략, WBS 기반으로 연계 가능한 구체적인 차주 업무 계획, 프로젝트 성공을 위한 중장기적 전략까지 제시
- **일반화되지 않고, 구체적으로 작성**하며 weekly_report에서 작성하지 않은 내용 위주로 작성
